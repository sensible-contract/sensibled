...
func UpdateBalanceInRedis(pipe redis.Pipeliner, blocksTotal int, addressBalanceCmds map[string]*redis.IntCmd, utxoToRestore, utxoToRemove map[string]*model.TxoData, isReorg bool) {
	...
	for outpointKey, data := range utxoToRestore {
		...
		if data.AddressData.CodeType == scriptDecoder.CodeType_NFT {
		...
		} else if data.AddressData.CodeType == scriptDecoder.CodeType_NFT_AUCTION {
		...
		} else if data.AddressData.CodeType == scriptDecoder.CodeType_NFT_SELL {
			...
			pipe.ZIncrBy(ctx, "{fb"+strGenesisId+strCodeHash+"}", float64(data.AddressData.SensibleData.FT.Amount), strAddressPkh) // ft:balance
			pipe.ZIncrBy(ctx, "{fs"+strAddressPkh+"}", float64(data.AddressData.SensibleData.FT.Amount), strCodeHash+strGenesisId) // ft:summary
