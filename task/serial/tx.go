...
func UpdateUtxoInRedis(utxoToRestore, utxoToRemove map[string]*model.TxoData, isReorg bool) (err error) {
	...
	for key, data := range utxoToRestore {
		...
		if len(data.GenesisId) < 20 {
			...
			// balance of address
			if err := pipe.IncrBy(ctx, "bl"+string(data.AddressPkh), int64(data.Satoshi)).Err(); err != nil {
				panic(err)
			}
			continue
		}
		...
		if err := pipe.IncrBy(ctx, "cb"+string(data.AddressPkh), int64(data.Satoshi)).Err(); err != nil {
			panic(err)
		}
		...
		if data.CodeType == scriptDecoder.CodeType_NFT {
			...
			// nft:utxo
			if err := pipe.ZAdd(ctx, "{nu"+string(data.AddressPkh)+"}"+string(data.CodeHash)+string(data.GenesisId),
			...
			// nft:owners
			if err := pipe.ZIncrBy(ctx, "{no"+string(data.GenesisId)+string(data.CodeHash)+"}",
			...
			// ft:utxo
			if err := pipe.ZAdd(ctx, "{fu"+string(data.AddressPkh)+"}"+string(data.CodeHash)+string(data.GenesisId),
				&redis.Z{Score: score, Member: key}).Err(); err != nil {
				panic(err)
			}
			// ft:balance
			if err := pipe.ZIncrBy(ctx, "{fb"+string(data.GenesisId)+string(data.CodeHash)+"}",
				float64(data.Amount),
	...
	for key, data := range utxoToRemove {
		...
		if len(data.GenesisId) < 20 {
			...
			// balance of address
			addressBalanceCmds["bl"+string(data.AddressPkh)] = pipe.DecrBy(ctx, "bl"+string(data.AddressPkh), int64(data.Satoshi))
			continue
		}

		// contract satoshi balance of address
		addressBalanceCmds["cb"+string(data.AddressPkh)] = pipe.DecrBy(ctx, "cb"+string(data.AddressPkh), int64(data.Satoshi))

		// redis有序genesis utxo数据清除
		if data.CodeType == scriptDecoder.CodeType_NFT {
			// nft:utxo
			if err := pipe.ZRem(ctx, "{nu"+string(data.AddressPkh)+"}"+string(data.CodeHash)+string(data.GenesisId),
				key).Err(); err != nil {
				panic(err)
			}
			// nft:utxo-detail
			if err := pipe.ZRem(ctx, "nd"+string(data.CodeHash)+string(data.GenesisId), key).Err(); err != nil {
				panic(err)
			}
			...
			// ft:utxo
			if err := pipe.ZRem(ctx, "{fu"+string(data.AddressPkh)+"}"+string(data.CodeHash)+string(data.GenesisId), key).Err(); err != nil {
				panic(err)
			}
			// ft:balance
			if err := pipe.ZIncrBy(ctx, "{fb"+string(data.GenesisId)+string(data.CodeHash)+"}",
		...
		// 记录key以备删除
		tokenToRemove[string(data.GenesisId)+string(data.CodeHash)] = true
